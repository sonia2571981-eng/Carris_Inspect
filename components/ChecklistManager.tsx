...
export const ChecklistManager: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'bus' | 'tram' | 'articulated'>('bus');
  ...
  useEffect(() => {
    getConfig().then(data => {
      // Garantir compatibilidade se a propriedade articulated ainda não existir na DB
      if (data && !data.tramArticulatedChecklist) {
        data.tramArticulatedChecklist = [];
      }
      setConfig(data);
    });
  }, []);
  ...
  const getCurrentList = () => {
    if (activeTab === 'bus') return config.busChecklist;
    if (activeTab === 'articulated') return config.tramArticulatedChecklist || [];
    return config.tramChecklist;
  };
  ...
  const updateConfigList = (newList: ChecklistConfigItem[]) => {
    setConfig(prev => {
        ...
        if (activeTab === 'bus') {
            newConfig.busChecklist = newList;
        } else if (activeTab === 'articulated') {
            newConfig.tramArticulatedChecklist = newList;
        } else {
            newConfig.tramChecklist = newList;
        }
